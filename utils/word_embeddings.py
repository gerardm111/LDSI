...
#model = fasttext.train_unsupervised('unlabeled/tokenized_sentences.txt', model='skipgram', dim=100, epoch=20, minCount=20)
#model.save_model("model_fasttext_4.bin")
## test on:
#model = fasttext.train_unsupervised('test_tokens.txt', model='cbow', dim=100, epoch=10)
#model.save_model("model_test.bin")

model = fasttext.load_model("./models/model_fasttext_2.bin")
